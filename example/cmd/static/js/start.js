...
let treeweight = `
<div class="input-label">{{item.title}}|
<el-input v-model="item.properties.weight" size="mini" min="0" max="9999" type="number" style="width: 44%"></el-input>
</div>
`

Vue.component('treeweight', {
    props: ['item'],
    template: treeweight
})
...
let stressDialog = new Vue({
    ...
    methods: {
        onClick() {
            ...
            this.robot.StressBehaviorTree(JSON.stringify(server),
             JSON.stringify(sinfo), 
             JSON.stringify(config.getSubtreeWeight()),
             JSON.stringify(config.getAppConfigJs()),
             ).
                then(() => {
        ...
        RunTree(tree){
            ...
            let svrcfg = config.GetServerConfig()
            svrcfg.account = tab.server.account

            stressDialog.robot.RunBehaviorTree(tree.id, tab.id, JSON.stringify(svrcfg))
                .then(() => {
                    config.saveLastloginServer()
                }).catch((e) => {
                    this.$notify({
